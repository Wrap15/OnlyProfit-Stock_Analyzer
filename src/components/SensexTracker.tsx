  ...
  loading: () => (
    <div className="w-full h-[280px] sm:h-[400px] bg-background/50 rounded-2xl border border-border flex items-center justify-center animate-pulse">
      <div className="flex flex-col items-center gap-2">
        ...
        <span className="text-xs text-text-secondary font-bold">Loading SENSEX Chart...</span>
      </div>
    </div>
  )
});
...
const SENSEX_CONSTITUENTS = [
  { symbol: 'HDFCBANK.NS', name: 'HDFC Bank Ltd.', weight: 14.00, sector: 'Financial Services' },
  { symbol: 'RELIANCE.NS', name: 'Reliance Industries Ltd.', weight: 10.70, sector: 'Energy' },
  { symbol: 'ICICIBANK.NS', name: 'ICICI Bank Ltd.', weight: 10.20, sector: 'Financial Services' },
  { symbol: 'BHARTIARTL.NS', name: 'Bharti Airtel Ltd.', weight: 6.50, sector: 'Communication Services' },
  { symbol: 'INFY.NS', name: 'Infosys Ltd.', weight: 5.50, sector: 'Information Technology' },
  { symbol: 'LT.NS', name: 'Larsen & Toubro Ltd.', weight: 5.00, sector: 'Industrials' },
  { symbol: 'SBIN.NS', name: 'State Bank of India', weight: 4.80, sector: 'Financial Services' },
  { symbol: 'TCS.NS', name: 'Tata Consultancy Services Ltd.', weight: 4.20, sector: 'Information Technology' },
  { symbol: 'AXISBANK.NS', name: 'Axis Bank Ltd.', weight: 3.80, sector: 'Financial Services' },
  { symbol: 'ITC.NS', name: 'ITC Ltd.', weight: 3.50, sector: 'Consumer Staples' },
  { symbol: 'KOTAKBANK.NS', name: 'Kotak Mahindra Bank Ltd.', weight: 3.00, sector: 'Financial Services' },
  { symbol: 'HINDUNILVR.NS', name: 'Hindustan Unilever Ltd.', weight: 2.80, sector: 'Consumer Staples' },
  { symbol: 'M&M.NS', name: 'Mahindra & Mahindra Ltd.', weight: 2.50, sector: 'Consumer Discretionary' },
  { symbol: 'TATAMOTORS.NS', name: 'Tata Motors Ltd.', weight: 2.30, sector: 'Consumer Discretionary' },
  { symbol: 'BAJFINANCE.NS', name: 'Bajaj Finance Ltd.', weight: 2.20, sector: 'Financial Services' },
  { symbol: 'SUNPHARMA.NS', name: 'Sun Pharmaceutical Industries Ltd.', weight: 2.10, sector: 'Health Care' },
  { symbol: 'HCLTECH.NS', name: 'HCL Technologies Ltd.', weight: 2.00, sector: 'Information Technology' },
  { symbol: 'NTPC.NS', name: 'NTPC Ltd.', weight: 1.80, sector: 'Utilities' },
  { symbol: 'POWERGRID.NS', name: 'Power Grid Corporation of India Ltd.', weight: 1.60, sector: 'Utilities' },
  { symbol: 'ADANIPORTS.NS', name: 'Adani Ports & SEZ Ltd.', weight: 1.40, sector: 'Industrials' },
  { symbol: 'INDUSINDBK.NS', name: 'IndusInd Bank Ltd.', weight: 1.40, sector: 'Financial Services' },
  { symbol: 'ZOMATO.NS', name: 'Zomato Ltd.', weight: 1.30, sector: 'Consumer Discretionary' },
  { symbol: 'TITAN.NS', name: 'Titan Company Ltd.', weight: 1.20, sector: 'Consumer Discretionary' },
  { symbol: 'ULTRACEMCO.NS', name: 'UltraTech Cement Ltd.', weight: 1.10, sector: 'Materials' },
  { symbol: 'MARUTI.NS', name: 'Maruti Suzuki India Ltd.', weight: 1.10, sector: 'Consumer Discretionary' },
  { symbol: 'TATASTEEL.NS', name: 'Tata Steel Ltd.', weight: 1.00, sector: 'Materials' },
  ...
  { symbol: 'BAJAJFINSV.NS', name: 'Bajaj Finserv Ltd.', weight: 0.70, sector: 'Financial Services' },
  { symbol: 'BEL.NS', name: 'Bharat Electronics Ltd.', weight: 0.60, sector: 'Industrials' }
];
...
export default function SensexTracker({ symbol, indexQuote, onBack }: SensexTrackerProps) {
  ...
  useEffect(() => {
    async function fetchConstituentQuotes() {
      try {
        setLoading(true);
        const symbolsParam = SENSEX_CONSTITUENTS.map(c => c.symbol).join(',');
        const res = await axios.get(`/api/stock/quote?symbols=${symbolsParam}`);
        const quotes = res.data || [];

        const populated = SENSEX_CONSTITUENTS.map(item => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 transition-colors duration-300 animate-fade-in space-y-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-text-secondary">BSE INDIA</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black text-text-primary tracking-tight">
              SENSEX INDEX TRACKER
            </h1>
            <p className="text-xs sm:text-sm text-text-secondary font-medium max-w-2xl leading-relaxed">
              Tracking the performance of 30 large, liquid, and financially sound Indian companies listed on the Bombay Stock Exchange.
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="p-4 bg-glass border border-border/80 rounded-2xl shadow-premium grid grid-cols-3 gap-2 text-xs items-center">
          <div>
            ...
            <span className="font-black text-text-primary mt-0.5 block text-sm">23.84</span>
          </div>
          <div>
            <span className="text-[9px] font-bold text-text-secondary uppercase tracking-wider block">P/B Ratio</span>
            <span className="font-black text-text-primary mt-0.5 block text-sm">3.95</span>
          </div>
          <div>
            <span className="text-[9px] font-bold text-text-secondary uppercase tracking-wider block">Div Yield</span>
            <span className="font-black text-profit mt-0.5 block text-sm">1.18%</span>
          </div>
        </div>
        ...
        <div className="p-4 bg-glass border border-border/80 rounded-2xl shadow-premium flex items-center justify-between gap-3 text-xs">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className="font-black text-text-primary mt-0.5 block">100 (Base: 1978-79)</span>
          </div>
        </div>
      </div>

      {/* Interactive SENSEX Chart Section */}
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2 auto-rows-[60px] sm:auto-rows-[80px]">
                {sortedConstituents.map((item) => {
                  ...
                  const isLarge = item.weight >= 8.0;
                  const isMedium = item.weight >= 4.0 && item.weight < 8.0;
      ...
      <div className="p-4 border border-border/60 bg-background rounded-2xl flex items-start gap-3 text-[10px] text-text-secondary leading-relaxed font-semibold">
        ...
        <div>
          <p>
            The S&P BSE SENSEX Index is computed by free-float market capitalization methodology where the level of the index reflects the total free-float market value of all the 30 constituent stocks relative to base period value.
          </p>
          <p className="mt-1">
            Simulated quotes are fetched dynamically and recalculated in real-time. Weight allocation figures are updated according to standard BSE India index circulars.
          </p>
        </div>
      </div>
